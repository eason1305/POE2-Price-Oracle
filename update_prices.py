...
def fetch_prices():
    ...
    data = http_json(
        ...
        f"?league={urllib.request.quote(str(league['id']))}&type=Currency"
    )
    core = data.get("core", {}) or {}
    items = norm_items(core.get("items"))

    def name_of(iid):
        meta = items.get(str(iid)) or {}
        return meta.get("name") or str(iid)

    primary_name = name_of(id_of(core.get("primary"))) or "primary"

    by_name = {}
    for line in data.get("lines", []) or []:
        by_name[name_of(line.get("id")).lower()] = line.get("primaryValue")
    ...
    for want in CURRENCIES:
        ...
        if v is None:
            ...
            rows.append((want, v))
    return league, primary_name, rows, missing
